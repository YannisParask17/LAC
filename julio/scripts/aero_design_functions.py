...
def get_design_functions(i_des_funs):
    ...
    if i_des_funs == 1:
        cl = [1.5138999999999958, 1.5138999999999958, 1.4326999999999999, 1.3989, 0.7425000000000018, 0.0, 0.0]
        cd = [0.014552013593356018, 0.014552013593356018, 0.017075689725789673, 0.021377442022081927, 0.03254612318627829, 0.6, 0.6]
        aoa = [10.127611178447252, 10.127611178447252, 9.435582501283362, 6.07329414569828, 3.0888580446417984, 0.0, 0.0]
    elif i_des_funs == 2:
        cl = [1.4139000000000002, 1.4139000000000002, 1.3327000000000164, 1.298900000000027, 0.6424999999999994, 0.0, 0.0]
        cd = [0.013462912738467942, 0.013462912738467942, 0.015682551073785765, 0.020596371772063263, 0.031826912205649135, 0.6, 0.6]
        aoa = [9.144287559414455, 9.144287559414455, 8.433574310138903, 5.2970754586135556, 2.508863954575325, 0.0, 0.0]
    elif i_des_funs == 3:
        cl = [1.3139000000000003, 1.3139000000000003, 1.2327000000000181, 1.1989000000000005, 0.5424999999999994, 0.0, 0.0]
        cd = [0.012579957352588572, 0.012579957352588572, 0.014718626752795042, 0.019964317086192902, 0.0316254735803436, 0.6, 0.6]
        aoa = [8.207286609806989, 8.207286609806989, 7.49309479479005, 4.558616035847117, 1.836019107496301, 0.0, 0.0]
    else:
        raise ValueError(f"i_des_funs={i_des_funs} is not a valid value. Should between 1-3.")

    return (
        PchipInterpolator(tc, cl),
        PchipInterpolator(tc, cd),
        PchipInterpolator(tc, aoa),
        np.array(tc[1:-2]), np.array(cl[1:-2]), np.array(cd[1:-2]), np.array(aoa[1:-2])
    )

def get_design_functions_1MW():
    ...
    cl = [0.9, 0.9, 0.8, 0.8, 0.7, 0.6, 0.0, 0.0]
...
def f_flow_fun(a, tsr, rt, B, R, use_HAWC2_poly=False):
...
def solve_tc(cl_des, r, t, tsr, R, a, B, tc_bounds=(0, 100), use_HAWC2_poly=False):
    ...
    for i, (_r, _t) in enumerate(zip(r, t)):
        ...
        try:
            ...
            raise Exception(f"solve_tc failed to find a solution. It failed at index={i} coresponding to a span location of r={_r:2.2f}")
    return tcs

def bem_residual(a, r, tsr, R, chord, cl, B, use_HAWC2_poly=False):
...
def solve_bem(r, tsr, R, chord, cl, B, a_bounds=(-1e-3, 0.9), use_HAWC2_poly=False):
    ...
    for i, (_r, _c, _cl) in enumerate(zip(r, chord, cl)):
        args = (_r, tsr, R, _c, _cl, B, use_HAWC2_poly)  # Arguments passed to the residual function
        try:
            a[i] = brenth(bem_residual, *a_bounds, args)
        except:
            raise Exception(f"solve_bem failed to find a solution. It failed at index={i} coresponding to a span location of r={_r:2.2f}")
    return a

def chord_fun(r, tsr, R, a, B, cl, use_HAWC2_poly=False):
